...
- A provider-retired model is now retired from Agora's registry. keycall 1.10.0 refuses a shut-down model before the network with a new `MODEL_RETIRED` code, distinct from `MODEL_NOT_AVAILABLE`; the retirement guard only caught the latter, so a stored model the provider later retired kept being offered and kept failing. The guard now retires on either code (keycall pin raised to `>=1.11.1`).
